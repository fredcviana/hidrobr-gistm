...
const SCORE_VALUES: Record<string,number> = { non_conforming:0, partially_conforming:50, conforming:75, fully_conforming:100 }
...
function RequirementModal({req,response,cycleId,onClose}:{req:any;response:any;cycleId:string;onClose:()=>void}) {
  ...
  const saveMut=useMutation({
    mutationFn:async(newStatus?:string)=>{
      const data={cycle_id:cycleId,requirement_id:req.id,implementation_text:text,responsible_person:responsible,sub_req_responses:subs,updated_at:new Date().toISOString()}
      if(newStatus){Object.assign(data,{status:newStatus,submitted_at:new Date().toISOString()})}
      if(response?.id){
        await supabase.from('requirement_responses').update(data).eq('id',response.id)
      } else {
        await supabase.from('requirement_responses').insert({...data,status:newStatus??'in_progress'})
      }
    },
    onSuccess:()=>{qc.invalidateQueries({queryKey:['requirements',cycleId]});onClose()},
  })

  const assessMut=useMutation({
    mutationFn:async()=>{
      await supabase.from('requirement_responses').update({status:'approved'}).eq('id',response.id)
      await supabase.from('hidrobr_assessments').insert({response_id:response.id,assessed_by:profile!.id,score:hidrScore,score_value:SCORE_VALUES[hidrScore]??0,assessment_text:hidrText,recommendations:hidrRec,published_at:new Date().toISOString()})
    },
    onSuccess:()=>{qc.invalidateQueries({queryKey:['requirements',cycleId]});onClose()},
  })

  const resp=response
  const hasAssessment=resp?.hidrobr_assessments?.length>0
  const assessment=resp?.hidrobr_assessments?.[0]

  return (
    <div className="fixed inset-0 bg-black/40 flex justify-end z-50" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="w-[580px] h-full bg-white flex flex-col shadow-2xl" onClick={e=>e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">
                  Lista de verificação ({subsDone}/{req.sub_requirements?.length??0})
                </div>
                ...
                <div className="bg-gradient-to-br from-brand-50 to-emerald-50 border border-brand-200 rounded-xl p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400">Publicada · Score: {assessment.score_value}/100</div>
                    </div>
          ...
          {tab==='comentarios'&&(
            <div className="text-center py-12 text-gray-400">
              ...
              <div className="font-medium text-gray-500 mb-4">Comentários em breve</div>
              <p className="text-xs">Este módulo será habilitado na próxima versão</p>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-2.5 px-5 py-3.5 border-t border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          {tab==='resposta'&&(resp?.status==='not_started'||!resp)&&(
            <button className="btn-primary btn-sm" onClick={()=>saveMut.mutate('in_progress')}>Iniciar preenchimento</button>
          )}
          {tab==='resposta'&&['in_progress','needs_revision'].includes(resp?.status)&&!hb&&(
            <button
              ...
              onClick={()=>saveMut.mutate('submitted')}
              disabled={saveMut.isPending||text.length<20}>
              {saveMut.isPending?<Loader2 className="w-3 h-3 animate-spin"/>:<Send className="w-3 h-3"/>}
              Solicitar avaliação
            </button>
          )}
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400">
          <div className="flex items-center gap-1.5">
            <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{width:`${pct}%`,background:topic.color_hex}}/>
            </div>
...
export function RequirementsPage() {
  ...
  const {data:cycleData}=useQuery({
    queryKey:['active-cycle',profile?.organization_id],
    enabled:!!profile?.organization_id||isHidrobr(profile?.role),
    queryFn:async()=>{
      let q=supabase.from('assessment_cycles').select('id,name').eq('status','active').order('created_at',{ascending:false}).limit(1)
      if(!isHidrobr(profile?.role)&&profile?.organization_id) q=q.eq('organization_id',profile.organization_id)
      const {data}=await q
      return data?.[0]
    }
  })
  ...
  if(!cycleId)return(
    <div className="p-6"><div className="card p-10 text-center">
      ...
      <p className="text-gray-400 text-sm mt-1">Contate a HIDROBR para criar um ciclo de avaliação</p>
    </div></div>
  )
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">18 princípios · {cycleData?.name}</p>
        </div>
      </div>
      <div className="flex gap-2 flex-wrap mb-5">
        {[['all','Todos (18)'],['approved','Aprovados'],['in_progress','Em andamento'],['submitted','Pend. avaliação'],['not_started','Não iniciados']].map(([v,l])=>(
      ...
      {selected&&<RequirementModal req={selected.req} response={selected.response} cycleId={cycleId!} onClose={()=>setSelected(null)}/>}
    </div>
  )
}
